	...
	Janus.init({
		...
		callback: function () {
			...
			$("#start").one("click", function () {
				...
				// Create session
				/**
				 * server: 소켓서버 주소를 가집니다. default: null / ref: settings.js
				 * iceServers : 비디오 룸에 참여 시 STUN/TURN 서버 주소를 가집니다. default: null / ref: janus.js:2176
				 */
				janus = new Janus({
					/**
					 * 아래 server를 wss로 변경시켜야 작동합니다. ref: settings.js
					 * (기본 코드에서 테스트를 위해 wss 주소로 변경 시켜두었습니다.)
					 */
					server: server,
					...
					success: function () {
						...
						/**
						 * janus.js의 Janus 객체 내부에
						 * this.attach = function (callbacks) {createHandle(callbacks);};
						 * 를 참조하셔야합니다.
						 *
						 * attach는 janus객체에 handler메서드와 dto성격의 getter/setter를
						 * 생성하는데 사용됩니다.
						 */
						janus.attach({
							...
							success: function (pluginHandle) {
								console.trace("pluginHandle: ", pluginHandle);
								$("#details").remove();
							...
							/**
							 * 유저네임 기입 후 비디오룸 입장 시 잠시 팝업되는 화살표이미지
							 */
							...
							/**
							 * icecandidate 상태 변화에 따른 상태 메세지 출력
							 * ref: janus.js:2239
							 */
											...
											if (msg["error_code"] === 426) {
												...
												bootbox.alert(
													"<p>Apparently room <code>" +
														myroom +
														"</code> (the one this demo uses for testing VP9 SVC) " +
...
/**
 * janus.attach에서 해당 함수를 호출하게 됩니다.
 *
 */
function registerUsername() {
	if ($("#username").length === 0) {
		...
		// Try a registration
		/**
		 * username이 빈 상태일 때 join 시 required를 표시합니다.
		 */
...
function addSvcButtons(feed) {
	...
	$("#remote" + index)
		...
		.append(
			'<div id="layers' +
				...
				'      <button id="sl' +
				index +
				'-2" type="button" class="btn btn-primary" data-toggle="tooltip" title="Switch to high resolution" style="width: 34%">SL 2</button>' +
				'      <button id="sl' +
				index +
				'-1" type="button" class="btn btn-primary" data-toggle="tooltip" title="Switch to normal resolution" style="width: 33%">SL 1</button>' +
				'      <button id="sl' +
				index +
				'-0" type="button" class="btn btn-primary" data-toggle="tooltip" title="Switch to low resolution" style="width: 33%">SL 0</button>' +
				"    </div>" +
				"  </div>" +
				'  <div class"row">' +
				'    <div class="btn-group btn-group-xs" style="width: 100%">' +
				'      <button id="tl' +
				index +
				'-2" type="button" class="btn btn-primary" data-toggle="tooltip" title="Cap to temporal layer 2 (high FPS)" style="width: 34%">TL 2</button>' +
				'      <button id="tl' +
				index +
				'-1" type="button" class="btn btn-primary" data-toggle="tooltip" title="Cap to temporal layer 1 (medium FPS)" style="width: 33%">TL 1</button>' +
				'      <button id="tl' +
				index +
				'-0" type="button" class="btn btn-primary" data-toggle="tooltip" title="Cap to temporal layer 0 (low FPS)" style="width: 33%">TL 0</button>' +
				"    </div>" +
				"  </div>" +
				"</div>"
		);
	...
	$("#sl" + index + "-0")
		...
		.click(function () {
			toastr.info(
				"Switching SVC spatial layer, wait for it... (low resolution)",
				null,
				{ timeOut: 2000 }
			);
			...
			$("#sl" + index + "-0")
				...
				.addClass("btn-info");
			feeds[index].send({
				message: { request: "configure", spatial_layer: 0 },
			});
		});
	$("#sl" + index + "-1")
		.removeClass("btn-primary btn-success")
		...
		.click(function () {
			toastr.info(
				"Switching SVC spatial layer, wait for it... (normal resolution)",
				null,
				{ timeOut: 2000 }
			);
			if (!$("#sl" + index + "-2").hasClass("btn-success"))
			...
			if (!$("#sl" + index + "-0").hasClass("btn-success"))
				$("#sl" + index + "-0")
					...
					.addClass("btn-primary");
			feeds[index].send({
				message: { request: "configure", spatial_layer: 1 },
			});
		});
	$("#sl" + index + "-2")
		.removeClass("btn-primary btn-success")
		...
		.click(function () {
			toastr.info(
				"Switching SVC spatial layer, wait for it... (high resolution)",
				null,
				{ timeOut: 2000 }
			);
			$("#sl" + index + "-2")
			...
			if (!$("#sl" + index + "-0").hasClass("btn-success"))
				$("#sl" + index + "-0")
					...
					.addClass("btn-primary");
			feeds[index].send({
				message: { request: "configure", spatial_layer: 2 },
			});
		});
	$("#tl" + index + "-0")
		.removeClass("btn-primary btn-success")
		...
		.click(function () {
			toastr.info(
				"Capping SVC temporal layer, wait for it... (lowest FPS)",
				null,
				{ timeOut: 2000 }
			);
			...
			$("#tl" + index + "-0")
				...
				.addClass("btn-info");
			feeds[index].send({
				message: { request: "configure", temporal_layer: 0 },
			});
		});
	$("#tl" + index + "-1")
		.removeClass("btn-primary btn-success")
		...
		.click(function () {
			toastr.info(
				"Capping SVC temporal layer, wait for it... (medium FPS)",
				null,
				{ timeOut: 2000 }
			);
			if (!$("#tl" + index + "-2").hasClass("btn-success"))
			...
			if (!$("#tl" + index + "-0").hasClass("btn-success"))
				$("#tl" + index + "-0")
					...
					.addClass("btn-primary");
			feeds[index].send({
				message: { request: "configure", temporal_layer: 1 },
			});
		});
	$("#tl" + index + "-2")
		.removeClass("btn-primary btn-success")
		...
		.click(function () {
			toastr.info(
				"Capping SVC temporal layer, wait for it... (highest FPS)",
				null,
				{ timeOut: 2000 }
			);
			$("#tl" + index + "-2")
			...
			if (!$("#tl" + index + "-0").hasClass("btn-success"))
				$("#tl" + index + "-0")
					...
					.addClass("btn-primary");
			feeds[index].send({
				message: { request: "configure", temporal_layer: 2 },
			});
		});
}
...
function updateSvcButtons(feed, spatial, temporal) {
	...
	if (spatial === 0) {
		...
		toastr.success("Switched SVC spatial layer! (high resolution)", null, {
			timeOut: 2000,
		});
		$("#sl" + index + "-2")
			.removeClass("btn-primary btn-info btn-success")
